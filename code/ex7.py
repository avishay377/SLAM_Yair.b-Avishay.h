...
# In ex7.py, update your import statement to this:
from algorithms_library import (read_images_from_dataset, extract_keypoints_and_inliers,
                                calculate_right_camera_matrix, cv_triangulate_matched_points,
                                find_consensus_matches_indices, read_cameras_matrices,
                                extract_actual_consensus_pixels, plot_trajectories_and_landmarks_ex7,
                                get_truth_transformation, compute_trajectory_left_cams,
                                compute_trajectory_gtsam_left_cams, convert_gtsam_cams_to_global,
                                read_poses_truth, compute_trajectory_left_cams, convert_gtsam_cams_to_global)
import cv2
...
def main_(db, poseGraph_saved=False):
    print("Main-Start")
    symbol_c = "c"
    ...
    for n in range(len(relative_poses)):  # n is kf num n -> frame n * 5
        ...
        if n in [3, 280, 320, 400, 480, 520, 600, 660, 671]:
            plot_optimized_values(poseGraph, loop=True, title=f"{n}")
    #
    ...
    organize_trajectories_for_plot(key_frames_range, relative_poses, poseGraph)
    q5(poseGraph, key_frames_range)
...
def calculate_and_plot_results(pose_graph, bundle_key_frames):
    # Get optimized and initial cams
    optimized_cams = pose_graph.get_optimized_cameras()
    initial_cams = pose_graph.get_initial_cameras()  # Trajectory without loops

    # Convert to Global 3D points
    optimized_3d = compute_trajectory_gtsam_left_cams(convert_gtsam_cams_to_global(optimized_cams))
    initial_3d = compute_trajectory_gtsam_left_cams(convert_gtsam_cams_to_global(initial_cams))

    # Ground Truth
    truth_trans = np.array(get_truth_transformation(num_frames=3360))[bundle_key_frames]
    truth_3d = compute_trajectory_left_cams(truth_trans)

    # A. Plot Absolute Location Error
    err_optimized = np.linalg.norm(optimized_3d - truth_3d, axis=1)
    err_initial = np.linalg.norm(initial_3d - truth_3d, axis=1)

    plt.figure()
    plt.plot(err_initial, label='Without Loop Closure')
    plt.plot(err_optimized, label='With Loop Closure')
    ...
    # B. Plot Uncertainty Size
    # Measure: Determinant of the marginal covariance matrix
    marginals = pose_graph.marginals()
    uncertainty_sizes = []
    for i in range(len(optimized_cams)):
        cov = marginals.marginalCovariance(symbol('c', i))
        # Using the determinant as a measure of the uncertainty "volume"
        uncertainty_sizes.append(np.linalg.det(cov))

    plt.figure()
    plt.plot(uncertainty_sizes)
    plt.title("Location Uncertainty Size (Determinant of Covariance)")
    plt.xlabel("Keyframe Index")
    plt.ylabel("Uncertainty Volume")
    plt.show()
...
def organize_trajectories_for_plot(key_frames, relative_poses_bundle, poseGraph):

    ground_truth_trans = np.array(get_truth_transformation())[key_frames]
    ground_truth = compute_trajectory_left_cams(ground_truth_trans)

    initial_est = db.initial_estimate_cams()[key_frames]

    bundle_cam_trans = convert_gtsam_cams_to_global(relative_poses_bundle)
    bundle_opt = compute_trajectory_gtsam_left_cams(bundle_cam_trans)


    gtsam_optimized_cameras_poses = poseGraph.get_optimized_cameras_poses()
    loop_opt = compute_trajectory_gtsam_left_cams(gtsam_optimized_cameras_poses)

    plot_trajectories(initial_est, bundle_opt, loop_opt, ground_truth)
    plot_trajectories_with_initial(initial_est, bundle_opt, loop_opt, ground_truth)


def plot_trajectories(initial_est, bundle_opt, loop_opt, ground_truth):
    """
    ...
    fig.savefig("results_ex7/Left cameras 2d trajectory of the all process.png")
    plt.close(fig)

def plot_trajectories_with_initial(initial_est, bundle_opt, loop_opt, ground_truth):
    """
    Compare the left cameras relative 2d positions to the ground truth
    """
    fig = plt.figure()
    ...
    fig.savefig("results_ex7/Left cameras 2d trajectory with initial estimate of the all process.png")
    plt.close(fig)


def q5(pose_graph, bundle_key_frames):
    # Get optimized and initial cams
    optimized_cams = pose_graph.get_optimized_cameras_poses()
    initial_cams = pose_graph.get_initial_cameras_poses()  # Trajectory without loop closures

    # Convert to Global 3D points
    optimized_3d = compute_trajectory_gtsam_left_cams(optimized_cams)
    initial_3d = compute_trajectory_gtsam_left_cams(initial_cams)

    # Ground Truth
    truth_trans = np.array(get_truth_transformation())[bundle_key_frames]
    truth_3d = compute_trajectory_left_cams(truth_trans)

    # 1. Plot Absolute Location Error (L2 Distance)
    err_optimized = np.linalg.norm(optimized_3d - truth_3d, axis=1)
    err_initial = np.linalg.norm(initial_3d - truth_3d, axis=1)

    plt.figure(figsize=(10, 6))
    plt.plot(err_initial, color='red', label='L2 distance before loop closure')
    plt.plot(err_optimized, color='blue', label='L2 distance after loop closure')
    ...
    # 2. Plot Uncertainty Size (Logarithmic Scale Base 2)
    marginals = pose_graph.marginals()
    uncertainty_sizes_final = []
    uncertainty_sizes_initial = []  # You may need to store initial marginals separately if desired

    for i in range(len(optimized_cams)):
        # Final optimized uncertainty
        cov_final = marginals.marginalCovariance(symbol('c', i))
        loc_cov_final = cov_final[3:6, 3:6]  # Bottom-right 3x3 for location [cite: 564]
        vol_final = np.sqrt(np.linalg.det(loc_cov_final))
        uncertainty_sizes_final.append(np.log2(vol_final) if vol_final > 0 else -np.inf)

    plt.figure(figsize=(10, 6))
    plt.plot(uncertainty_sizes_final, color='blue', label='Final uncertainty')
...
if __name__ == '__main__':
    ...
    # Plot Trajectories vs Ground Truth [cite: 509]

    # Plot Error and Uncertainty [cite: 540, 561]
